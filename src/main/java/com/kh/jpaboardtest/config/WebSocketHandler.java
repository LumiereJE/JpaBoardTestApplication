...
public class WebSocketHandler extends TextWebSocketHandler { // TextWebSocketHandler 또는 BinaryWebSocketHandler라는 스프링이 제공하는 기본 클래스를 상속하여 구현하면 됨
    ...
    // WebSocketSession : 웹소켓이 연결될 때 생기는 연결 정보를 담고있는 객체, 핸들러에서 웹소켓 통신에 대한 처리를 하기 위해 이 세션들을 컬렉션으로 담아서 관리하는 경우가 많음
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        ChatMessageDto chatMessage = objectMapper.readValue(payload, ChatMessageDto.class); // JSON을 객체로 Mapping해주고 ChatMessageDto로 받음
    ...
    @Override
    // WebSocket 연결의 생명주기를 관리하는데 있어 중요한 부분을 담당함, 연결 종료 직후에 호출
