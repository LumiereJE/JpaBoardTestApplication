...
public class MemberService {
    ...
    public List<MemberResDto> getMemberList() {
        ...
        for (Member member : members) {
            memberDtos.add(convertEntityToDto(member)); // 아래에 회원entity를 회원DTO로 변환한 것
        }
        return memberDtos;
    }
    ...
    // 회원 가입 여부 확인 (repository)
    public boolean isMember(String email) {
        return memberRepository.existsByEmail(email);
    }


    // JWT 추가되면서 AuthService로 옮겨짐(보안이 필요한 부분이라서?)
//    // 회원 가입
...
//    }
    ...
    public boolean deleteMember(String email) {
        try {
            ...
            return true;    // 회원이 존재하면 true반환
        } catch (RuntimeException e) {
            return false;       // 회원이 존재하지 않으면 false반환
        }
    }
