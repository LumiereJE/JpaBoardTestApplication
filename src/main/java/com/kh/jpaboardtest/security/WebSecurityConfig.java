...
// 기본적으로 스프링 시큐리티에 필요한 설정 → jwt적용 및 authentication에 필요한 API주소를 설정함
@RequiredArgsConstructor
@Configuration
@EnableWebSecurity // 스프링 시큐리티를 활성화하고 웹 보안 설정을 구성하는데에 사용함
@Component
public class WebSecurityConfig implements WebMvcConfigurer {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .and()
                .authorizeRequests()
                .antMatchers("/auth/**", "/ws/**", "/movies/**", "/test/**").permitAll() // 해당 항목 권한 풀어줌
