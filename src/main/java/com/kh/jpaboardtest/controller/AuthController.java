...
import com.kh.jpaboardtest.service.AuthService;
...
@Slf4j
@RestController // controller + ResponseBody → JSON 형태로 객체 데이터를 반환 하는 것, 데이터를 응답으로 제공하는 REST API를 개발할 때 주로 사용하며, 객체를 ResponseEntity로 감싸서 반환함
...
public class AuthController {   // 프론트와 백 연결해서 받아옴, 프레젠테이션 레이어, 웹 요청과 응답을 처리함
    ...
    // 회원가입
    @PostMapping("/signup") // 주어진 url표현식과 일치하는 HTTP POST요청을 처리(추가/등록)
    ...
    // accessToken 재발급
    // 리프레쉬 토큰을 받으면 백이 DB에 있는 토큰을 가져와서 이거랑 맞는지 확인함 -> 정말로 위/변조가 안일어났는지 확인해야 됨
    @PostMapping("/refresh")
