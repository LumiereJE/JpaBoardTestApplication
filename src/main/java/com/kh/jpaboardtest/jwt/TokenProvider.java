...
// 유저 정보로 jwt access/refresh 토큰 생성 및 재발급 + 토큰으로부터 유저 정보를 받아옴

@Slf4j
@Component // Bean Configuration 파일에 Bean을 따로 등록하지 않아도 사용할 수 있음
...
public class TokenProvider {
    ...
    private static final long ACCESS_TOKEN_EXPIRE_TIME = 1000 * 60 * 3; // 10분마다 만료
    private static final long REFRESH_TOKEN_EXPIRE_TIME = 1000 * 60 * 60 * 24 * 7L; // 재발급
    private final Key key; // 토큰을 서명하기 위한 Key

    // 주의점 : @Value 어노테이션은 springframework의 어노테이션이다.롬복의 @Value와 착각 xx
    public TokenProvider(@Value("${jwt.secret}") String secretKey) { // jwt.secret 값을 가져와서 JWT를 만들 때 사용하는 암호화 키값을 생성함
